...
export async function scoreWord(audioBlob, phonemes, language = 'en-US', options = {}) {
  ...
  const { getScoringMode } = await import('./utils/scoring/scoringConfig.js')
  const mode = await getScoringMode()
  if (mode === 'azure') return scoreAzure()
  const { shouldAssessStress, assessWithStress, assessSpeechSuperOnly } = await import('./utils/scoring/speechSuperStress.js')
  if (mode === 'speechsuper') return assessSpeechSuperOnly(audioBlob, referenceText)
  // both: Azure + chồng trọng âm SpeechSuper (chỉ khi từ ≥2 âm tiết)
