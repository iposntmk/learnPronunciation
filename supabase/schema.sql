...
create table public.sentence_pronunciation_attempts (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  sentence_id uuid not null references public.sentences(id) on delete cascade,
  score numeric(5,2) not null check (score >= 0 and score <= 100),
...
create index sentences_topic_idx on public.sentences(topic);
create index sentences_language_idx on public.sentences(language);
create index sentences_normalized_sentence_idx on public.sentences(normalized_sentence);
